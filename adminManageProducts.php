...
class Process
{
    public function checkAdmin()
    {
        if ($_SESSION["admin"]) {
            ...
            if ($admin) {
                ...
                ManageProductTemplete::generate($products);
            } else {
                include "404.php";
            }
        } else {
    ...
    {
        $result = $this->search("SELECT * FROM `product`");
                                            ...
                                            ?>
                                                <tr class="border-[#AD1212]" onclick="window.location.href = 'adminSingleProduct?id='+ <?php echo $product['id']; ?>">
                                                    ...
                                                    <td>
                                                        ...
                                                        if ($status == 1) {
                                                        ?>
                                                            <div onclick="ActivateOrDeactivateProduct(2, '<?php echo $product['id']; ?>');" class="bg-[#F7CECE] text-center p-1 rounded hover:cursor-pointer">
                                                                <span class="text-[#E63535]">Deactivate</span>
                                                            </div>
                                                        <?php
                                                        } else {
                                                        ?>
                                                            <div onclick="ActivateOrDeactivateProduct(1,'<?php echo $product['id']; ?>');" class="bg-[#C9F0DD] text-center p-1 rounded hover:cursor-pointer">
