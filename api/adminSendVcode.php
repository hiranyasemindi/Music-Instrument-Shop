...
class Process
{
    ...
    {
        $email = $_GET['email'];
        $admin = $this->searchAdminByEmail($email);
        if ($admin) {
            $otp = str_pad(rand(0, 9999), 4, '0', STR_PAD_LEFT);
            $this->updateVerificationCode($otp, $email);
            $body = "<h1>Sonority Admin Change Email Verification Code</h1>
            <h4>" . $otp . "</h4>";
    ...
    private function changeEmail($decoded)
    {
        $email = $decoded->email;
        $vcode = $decoded->vcode;
        $name = $decoded->name;
        $oldEmail = $decoded->oldEmail;
        $admin = $this->getAdminrByEmailAndVerificationCode($oldEmail, $vcode);
        if ($admin) {
            $this->updateProfile($email, $name, $oldEmail);
            $admin = $this->searchAdminByEmail($email);
            if ($admin) {
                session_start();
                ...
                $this->responseObj->error = "Admin not Found";
                $this->sendReponse(400);
            }
        } else {
    ...
    private function getAdminrByEmailAndVerificationCode($email, $vcode)
    {
        $result =  $this->search("SELECT * FROM `admin` WHERE `email`='" . $email . "' AND `vcode`='" . $vcode . "'");
        return $result->num_rows > 0 ? $result->fetch_assoc() : null;
    }

    private function updateProfile($email, $name, $oldEmail)
    {
        $this->iud("UPDATE `admin` SET `name`='" . $name . "', `email`='" . $email . "' WHERE `email`='" . $oldEmail . "' ");
    }
}
